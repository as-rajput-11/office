...
for neartostation in kl:
    ...
    while near_station:
        ...
        if station0[0][1] > countp:
            ...
            if len(near_station ) <= 1:
                # print(neartostation,"123456")
            
                timingchange.append(neartostation)
                cur.execute("select  train_id,arrival_time +interval'10 hour ' as addtime ,detraining_station from train_rpt where detraining_station = '"+str(neartostation[3])+"' order by arrival_time ")
                data4 = cur.fetchall()
                # print("+++",data4)
                # my_date_str = '{}-{}-{}'.format(*data4[1])
                # date_obj = datetime.datetime.strptime(my_date_str, '%Y-%m-%d %H:%M:%S')


                # data4 = datetime.datetime.strptime(data4[1], '%Y-%m-%d %H:%M:%S')
                # print(type(date_obj[1]))
                # print(neartostation[1])
                # time_dif = data4[1]-neartostation[1]
