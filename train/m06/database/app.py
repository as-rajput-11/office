...
import itertools
import json
conn = psycopg2.connect(host='127.0.0.1', dbname= "train", user= "postgres", password= "postgres")
app = Flask(__name__)
@app.route('/')
def index():
    train_details = []
...
def do_cluster():
    ...
    cur = conn.cursor()
    try:
        # changes according to column names in table "mst_geojson_100km"
        postgres_select_query = "SELECT id, station, capacity, y, x, geometry, entraning_ids, entraining_station, detraining_ids, detraining_station  FROM mst_geojson_100km order by id" 
    ...
    rows = cur.fetchall()
    cur.close()
    station_capacity_main_dict = {}

    i = 0
    column_name = ['id', 'station', 'capacity', 'y', 'x', 'geometry', 'entraining_ids', 'entraining_station', 'detraining_ids', 'detraining_station']
    for tup in rows:
        ...
        for j in range(len(tup)):
            # if(tup[j] != None):
                local_dict[column_name[j]] = tup[j] 
        

        station_capacity_main_dict["dict" + str(i)] = (local_dict)
        i = i + 1

    feature_list = [] #list of all features list 

    for val in station_capacity_main_dict.values():
        # adding feature same as geojson
        feature = { 
                "type": "Feature",
                "properties": {
                    "id": val['id'],
                    ...
                    "ENTRAINING_STATION":val['entraining_station'],
                    "ENTRAINING_IDS":val['entraining_ids'],
                    "DETRAINING_STATION":val['detraining_station'],
                    "DETRAINING_IDS":val['detraining_ids'],
                    "y": val['y'],
                    "x": val['x']
                },
                "geometry": {
    ...
    # print(station_geojson);

    #  change file saving name as requirement
    with open('static/test.geojson', 'w') as json_file:
        json.dump(station_geojson, json_file)
