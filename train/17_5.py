...
for k, g in itertools.groupby(kl, lambda x: x[3]):
        ...
        for item in range(len(g)):
            try:
                ...
                h = data4[item][1]-g[item][1]
                print(g[item][0])

                # print(h)
                cur.execute("update trains set delay_time = start_time +interval'"+str(h)+"' where train_id= "+str(g[item][0])+"")
                conn.commit()
                ...
                h = data4[item][1]-data4[item-1][1]

                # print(h)
    
    # print(k, list(g))

#     late_train.append(list(g))
# print(late_train)
